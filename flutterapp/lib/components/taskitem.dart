...
class _TaskItemState extends State<TaskItem> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.showProject) {
      attributes.add(
        ProjectBadge(
          ...
          color: widget.task.projectColor,
        ),
      );
    }
    if (widget.showDate && widget.task.dueOn != null) {
      attributes.add(
        DueOn(
          dueOn: widget.task.dueOn,
          evening: widget.task.evening,
          showIcon: true,
        ),
      );
    }
      ...
      attributes.add(
        Wrap(
          ...
          children: [
            ...
            Text(
              "${widget.task.completeSubtaskCount}/${widget.task.subtaskCount}",
            ),
          ],
        ),
      );
    ...
    return ListTile(
      ...
      onTap: widget.showRestore
          ? null
          : () {
              Navigator.pushNamed(
                context,
                Routes.taskDetails,
                arguments: TaskDetailsArguments(widget.task),
              );
            },
    );
  }
...
class TaskActions extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
      ...
      showConfirmDelete(
        ...
        onConfirm: () async {
          ...
          try {
            ...
            messenger.showSnackBar(
              successSnackBar(theme: theme, text: 'Task Deleted'),
            );
            navigator.pop();
          } catch (e) {
            messenger.showSnackBar(
              errorSnackBar(theme: theme, text: 'Could not delete task'),
            );
          }
        },
      );
    ...
    Future<void> handleReschedule() async {
      var result = await showChangeDueOnDialog(
        context,
        task.dueOn,
        task.evening,
      );
