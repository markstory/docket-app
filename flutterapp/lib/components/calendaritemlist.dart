        ...
        child: Container(
          ...
          child: Column(
            ...
            children: calendarItems
                .map((item) => CalendarItemTile(calendarItem: item))
                .toList(),
          ),
        ),
      ),
...
class CalendarItemTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (startTime != null) {
      timeWidget = Text(paddedTime(startTime), style: TextStyle(color: color));
    }
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(calendarItem.title, overflow: TextOverflow.ellipsis),
          ),
        ],
      ),
    );
